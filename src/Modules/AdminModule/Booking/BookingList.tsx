...
export default function BookingList() {
  ...
  const tableCols = [
    ...
    t("Bookings_table_head.price"),
    t("Bookings_table_head.start_date"),
    t("Bookings_table_head.end_date"),
    t("Bookings_table_head.user"),
    "",
  ];
  ...
  const getBookings = async (pageNumber: number) => {
    try {
      setIsLoading(true);
      const response = await axiosInstance(BOOKING_URLS.GET_ALL, {
        params: {
          page: pageNumber,
          size: 10,
        },
    ...
    <>
      ...
      >
        <Table aria-label="booking list">
          ...
          {!isLoading && (
            <TableBody>
              {bookingList.map((booking: BookingListInterface) => (
                <TableRow
                  key={booking?._id}
                  sx={{
                    "&:nth-of-type(even)": {
